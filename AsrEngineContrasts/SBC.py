...
class Sbc(object):
    ...
    def _resume_read_case(self):
        sql = EngineDb()
        done = sql.select_done_id(self.taskname, 'sbc')
    ...
    def _resume_none_read_case(self):
        sql = EngineDb()
        done = sql.select_run_none_id(self.taskname, 'sbc')
        index = 1
        self.case_num = len(done)
        print('数量:', self.case_num)
        while True:
    ...
    def resume(self):
        self.start()
        sql = EngineDb()
        done = sql.count_done_case(self.taskname, 'sbc')
...
if __name__ == '__main__':
    sbc = Sbc('2020914_test', '/mnt/20200831_wav')
    sbc.start()
